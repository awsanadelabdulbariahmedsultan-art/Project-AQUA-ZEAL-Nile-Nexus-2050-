...
```mermaid
graph TD
    A[Raw Moisture-Rich Water Hyacinth & Papyrus] --> B[Primary Pre-treatment & Mechanical Dewatering]
    B -->|Organic Plant Press-Juice & Liquids| C[Anaerobic Digester Blocks]
    B -->|Dry Biomass Fibers: Moisture < 15%| D[Fast Pyrolysis Fluidized Bed Reactor]
    D -->|Condensable Bio-Oils & Synthesis Vapor| E[Catalytic Hydrotreating & Upgrading]
    D -->|Non-Condensable High-Heat Gases| B
    E --> F[Ultra-Pure Green Biodiesel Fuel]
    C --> G[Biomethane Gas + 8 MW Baseline Power Generation]
...
```mermaid
graph TD
    A[حشائش ورد النيل والبردي الرطبة] --> B[خط التجهيز الأولي والتجفيف الميكانيكي]
    B -->|عصير ومستخلصات نباتية رطبة| C[هاضم التخمير اللاهوائي]
    B -->|كتلة حيوية جافة ورطوبة < 15%| D[وحدة التحلل الحراري السريع Pyrolysis]
    D -->|أبخرة وزيوت حيوية خام| E[وحدة الهدرجة والترقية Hydrotreating]
    D -->|غازات حارة غير متكثفة| B
    E --> F[ديزل حيوي أخضر فائق النقاء]
    C --> G[غاز الميثان الحيوي + 8 ميجاوات كهرباء]
    style F fill:#2ecc71,stroke:#27ae60,stroke-width:2px
    style G fill:#3498db,stroke:#2980b9,stroke-width:2px
```
