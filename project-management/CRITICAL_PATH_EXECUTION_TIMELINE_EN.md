...
```mermaid
gantt
    title Integrated Jonglei Canal & Biorefinery Gantt Chart (24 Months)
    dateFormat  X
    axisFormat Month %m
    
    section Phase I: Mobilization & Readiness
    Refurbish "Toot We-She" Harvester & Machinery Deployment  :active, p1, 0, 6
    Fabricate Pyrolysis Process Lines at AOI Factories Egypt :active, p2, 1, 5
    Topographic Survey & Mapping for Infrastructure Coordinates :active, p3, 2, 6
    
    section Phase II: Parallel Deployment
    Hydraulic Cutter Suction Dredging for Final 100 km Segment   :crit, p4, 6, 18
    Construct Juba & Malakal Refineries & Pre-dry Biomass       :p5, 6, 15
    Build Schools, Clinics, and Engineered Livestock Crossings  :p6, 7, 18
    
    section Phase III: Commissioning & Linking
    Trial Runs, Biofuel Distribution & Grid Synchronization     :p7, 18, 22
    Official Water Release Across the Complete 360 km Canal Path :crit, p8, 22, 24
    Clear Liquidity Accounts & Pay Off CBE Bridge Loan           :p9, 22, 24
```
...
```mermaid
graph TD
    A[Month 1: MOFA Grant 15M + CBE Emergency Bridge Facility 30M] -->|Immediate Capital Outlay| B[Erect On-site Civil & Infrastructure Assets]
    B --> C[Month 18: Commercial Biorefinery Plant Commissioning]
    C -->|Steady Operating Net Revenues Generated: 16.2M/year| D[Month 24+: Automated Profit Capture & Transfer to CBE]
    D -->|Full Debt Amortization & Liability Clearance| E[100% Economically Independent & Self-Sustaining Nexus]
    style A fill:#34495e,stroke:#2c3e50,stroke-width:2px,color:#fff
    style C fill:#2ecc71,stroke:#27ae60,stroke-width:2px,color:#fff
    style E fill:#d35400,stroke:#e67e22,stroke-width:2px,color:#fff
```

### B. Integrated On-Site Operations (Final 100 km Path Layout)
```mermaid
graph LR
    subgraph Final 100 km Canal Line Cut Into 4 Parallel 25 km Blocks
    Q1[Sector A] --- Q2[Sector B] --- Q3[Sector C] --- Q4[Sector D]
    end
    
    subgraph Simultaneous Field Workflows Per Sector
    F1[Dredging Squads & Hydraulic Cutters] -->|Followed Immediately By| F2[Civil Infrastructure & Livestock Crossings]
    F2 -->|Logistical Fuel Linking| F3[Machinery Refueled via Malakal Green Biodiesel]
    end
...
```mermaid
gantt
    title مخطط غانت التنفيذي لمنظومة جونقلي والحشائش (24 شهراً)
    dateFormat  X
    axisFormat %m شهر
    
    section المرحلة الأولى: الحشد والجهوزية
    تأهيل الحفار توط وشي وصيانة الآليات      :active, p1, 0, 6
    تصنيع خطوط التحلل الحراري بالهيئة العربية    :active, p2, 1, 5
    المسح الطبوغرافي وتحديد مواقع المحطات والمدارس :active, p3, 2, 6
    
    section المرحلة الثانية: التنفيذ المتوازي
    أعمال الشق الهيدروليكي للـ 100 كم المتبقية    :crit, p4, 6, 18
    إنشاء مصانع جوبا وملكال وتشوين الحشائش   :p5, 6, 15
    بناء المدارس والمستشفيات وجسور الماشية       :p6, 7, 18
    
    section المرحلة الثالثة: التشغيل والربط
    التشغيل التجريبي وضخ الوقود الحيوي والمياه :p7, 18, 22
    إطلاق المياه رسميًا في مجرى القناة بالكامل   :crit, p8, 22, 24
    استلام وإغلاق مديونية البنك المركزي المصري   :p9, 22, 24
```
...
```mermaid
graph TD
    A[الشهر 1: منحة الخارجية 15M + اعتماد المركزي الجسري 30M] -->|صرف فوري للمقاولين| B[بدء البناء والإنشاءات بالموقع فوراً]
    B --> C[الشهر 18: بدء التشغيل الفعلي لمصانع الحشائش]
    C -->|تدفق أرباح تشغيلية فورية بقيمة 16.2M سنوياً| D[الشهر 24 وما بعده: اقتطاع آلي للأرباح لصالح البنك المركزي]
    D -->|إغلاق كامل القرض والتزامات التمويل| E[مشروع تنموي مستقل ماليًا ومستدام ذاتيًا 100%]
    style A fill:#34495e,stroke:#2c3e50,stroke-width:2px,color:#fff
    style C fill:#2ecc71,stroke:#27ae60,stroke-width:2px,color:#fff
    style E fill:#d35400,stroke:#e67e22,stroke-width:2px,color:#fff
```
...
```mermaid
graph LR
    subgraph خط الـ 100 كم المتبقي مقسم إلى 4 قطاعات متوازية 25 كم لكل قطاع
    Q1[القطاع أ] --- Q2[القطاع ب] --- Q3[القطاع ج] --- Q4[القطاع د]
    end
    
    subgraph هيكلية العمل المتوازي داخل كل قطاع
    F1[فرقة الحفر المائي وشق المجرى] -->|يتبعها مباشرة| F2[فرقة الإنشاءات المدنية وجسور الماشية]
    F2 -->|تكامل لوجستي للوقود| F3[تغذية الآليات بالديزل الأخضر من مصنع ملكال]
    end
